...
class BaseText(ABC):
    ...
    def __init__(self, prompt_builder, executor, context_builder, llm_concurrency, validator_func):
    ...
    def on_error(self, step_name: str, error: Exception) -> LLMResponse:
        ...
        """
        logger.error(
            f"[步骤异常] step={step_name} | err={error}",
            exc_info=True,
    ...
    async def _execute_single_task(self, task, injection_params: Dict[str, Any]) -> LLMResponse:
        ...
        # 基于当前输入文本长度动态计算 max_tokens（resolve_max_tokens 内含 4096 兜底）
        params[ke.KEY_MAX_TOKENS] = resolve_max_tokens(len(current_text))
        logger.debug(f"步骤 {step_name} max_tokens 设置为 {params[ke.KEY_MAX_TOKENS]}", module_name=self.CHINESE_NAME)
