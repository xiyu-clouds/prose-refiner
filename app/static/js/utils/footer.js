(function () {
  ...
  const CACHE_KEY_FOOTER_BG = 'card_config_footer_bg';

  function setFooterBg(url) {
    footer.style.backgroundImage = `url("${url}")`;
    footer.style.backgroundSize = 'cover';
    footer.style.backgroundPosition = 'center top';
    footer.style.backgroundRepeat = 'no-repeat';
  }

  async function loadFooterBg() {
    const cachedUrl = window.AppCache?.get(CACHE_KEY_FOOTER_BG);
    if (cachedUrl) {
      setFooterBg(cachedUrl);
    }
    ...
    try {
      ...
      let targetUrl;
      if (data.footer_bg_image_url) {
        targetUrl = data.footer_bg_image_url;
      } else {
        targetUrl = '/media/image/166.png';
      }
      window.AppCache?.set(CACHE_KEY_FOOTER_BG, targetUrl, 1800);
      if (cachedUrl !== targetUrl) {
        setFooterBg(targetUrl);
      }
    } catch (e) {
      if (!cachedUrl) {
        setFooterBg('/media/image/166.png');
      }
    }
  }
  loadFooterBg();
